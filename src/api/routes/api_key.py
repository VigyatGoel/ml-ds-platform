...
async def generate_key(owner: str, db: AsyncSession = Depends(db_service.get_db_service)):
    return await db_service.generate_key_service(db, owner)


@router.delete("/delete-key/{api_key}")
async def delete_key(api_key: str, db: AsyncSession = Depends(db_service.get_db_service)):
